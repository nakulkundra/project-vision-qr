...
## 2023-10-24 - Weak random number generation
**Vulnerability:** Predictable random numbers generated using Math.random() for session IDs and seeds.
**Learning:** Predictable session IDs can make the application vulnerable to session hijacking. The Web Crypto API provides cryptographically secure random number generation but may not be available in all environments, requiring a robust feature-detection fallback.
**Prevention:** Always use cryptographically secure methods like crypto.getRandomValues for generating security-sensitive random numbers, and include a fallback for environments where it's unavailable.
