...
## 2026-08-16 - Secure Random Generation with Web Crypto API
**Vulnerability:** Weak random number generation using `Math.random()` was used for `sessionId` and `_seed`.
**Learning:** Predictable `Math.random()` values can lead to predictable stream patterns. When replacing it with `crypto.getRandomValues()`, we must ensure robust feature-detection fallbacks and preserve original bitwise layout constraints.
**Prevention:** Always use cryptographically secure random number generators (e.g., Web Crypto API) for security-sensitive or unique identifiers, while maintaining fallback support for environments without native crypto.
