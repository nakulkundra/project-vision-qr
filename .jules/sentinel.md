...
## 2026-08-23 - Preserving bitwise constraints during CSPRNG migration
**Vulnerability:** Predictable session IDs and seeds generated via Math.random().
**Learning:** The fountain code layer relies on specific bitwise layout constraints (e.g., zeroed lower bits for the seed: `(rand << 16) >>> 0`). Failing to preserve these constraints when upgrading to `crypto.getRandomValues` causes silent regressions in packet reconstruction.
**Prevention:** Always ensure drop-in CSPRNG replacements output values in the exact same range and bitwise format as the original weak PRNG, especially in mathematical or codec-heavy contexts.
